...
import { foundItemsService, categoriesService } from '../../services/api';
...
const foundItemSchema = z.object({
  ...
  lokasi_ditemukan: z.string().min(5, 'Lokasi minimal 5 karakter'),
  tanggal_ditemukan: z.string().min(1, 'Tanggal wajib diisi'),
  foto: z
    .any()
    .optional()
...
export default function FoundItemForm() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await foundItemsService.create(payload, data.foto?.[0]);
  ...
  return (
    <div className="max-w-2xl mx-auto pb-10">
      ...
      <div className="bg-card border border-border rounded-2xl shadow-sm p-6 md:p-8">
        <h1 className="text-2xl font-bold mb-1">Laporkan Barang Ditemukan</h1>
        <p className="text-muted-foreground text-sm mb-8">Bantu kembalikan barang kepada pemiliknya dengan melaporkan apa yang Anda temukan.</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <div className="space-y-2">
            ...
            <input
              type="text"
              placeholder="Cth: Kunci Motor Honda"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="space-y-2">
              <label className="text-sm font-medium">Lokasi Ditemukan *</label>
              <input
                type="text"
                placeholder="Cth: Parkiran Gedung A"
                className={`w-full px-4 py-2.5 bg-background border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 ${errors.lokasi_ditemukan ? 'border-destructive' : 'border-input'}`}
                {...register('lokasi_ditemukan')}
              />
              {errors.lokasi_ditemukan && <p className="text-xs text-destructive">{errors.lokasi_ditemukan.message}</p>}
            </div>

            <div className="space-y-2">
              <label className="text-sm font-medium">Tanggal Ditemukan *</label>
              <input
                type="date"
                className={`w-full px-4 py-2.5 bg-background border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 ${errors.tanggal_ditemukan ? 'border-destructive' : 'border-input'}`}
                {...register('tanggal_ditemukan')}
              />
              {errors.tanggal_ditemukan && <p className="text-xs text-destructive">{errors.tanggal_ditemukan.message}</p>}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              rows={4}
              placeholder="Jelaskan kondisi, warna, merek, dan ciri-ciri barang yang Anda temukan..."
          ...
          <div className="pt-4">
            ...
            >
              {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Laporkan Temuan'}
            </button>
          </div>
        </form>
      </div>

...
function FoundItemCard({ item }) {
  ...
  const statusLabels = { published: 'Tersedia', claimed: 'Sudah Diklaim' };
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
          <Calendar size={12} />
          <span>{new Date(item.tanggal_ditemukan).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
        </div>
        <Link
...
export default function FoundItems() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      setLoading(true);
      try {
        const { data, count } = await foundItemsService.getAll({ search, categoryId, page, limit });
        setItems(data || []);
        setTotal(count || 0);
      } catch (err) {
        console.error(err);
  ...
  return (
    <div className="flex flex-col gap-6 pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border pb-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Barang Ditemukan</h1>
          <p className="text-muted-foreground mt-1">{total} barang menunggu diklaim</p>
        </div>
        {user && (
          <Link
            to="/items/found/new"
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center py-20 text-center">
          ...
          <p className="text-muted-foreground">Tidak ada barang ditemukan yang cocok.</p>
        </div>
      ) : (

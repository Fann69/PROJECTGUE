...
import { foundItemsService } from '../../services/api';
...
export default function ManageFoundReports() {
  ...
  const fetchReports = async () => {
    setLoading(true);
    try {
      const { data } = await foundItemsService.getAll({ limit: 100 });
      setReports(data || []);
    } catch (err) {
      toast.error('Gagal memuat data Laporan Barang Temuan.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAction = async (reportId, newStatus, reporterName, itemName) => {
    const isClaimed = newStatus === 'claimed';
    
    const title = isClaimed ? 'Tandai Sudah Diklaim?' : 'Publikasikan Ulang?';
    const text = isClaimed 
      ? `Laporan barang temuan "${itemName}" akan ditandai sebagai sudah diklaim.`
      : `Laporan barang temuan "${itemName}" akan dikembalikan ke status aktif.`;
    
    const confirmColor = isClaimed ? 'hsl(270, 70%, 50%)' : 'hsl(217, 91%, 60%)';
    const confirmText = isClaimed ? 'Ya, Tandai!' : 'Ya, Publikasikan!';
    ...
    if (result.isConfirmed) {
      try {
        await foundItemsService.updateStatus(reportId, newStatus, reporterName, itemName);
        toast.success(
          isClaimed ? 'Barang ditandai sebagai sudah diklaim.' : 'Barang dipublikasikan ulang.'
        );
        fetchReports();
      } catch (err) {
  ...
  const handleDelete = async (reportId, itemName) => {
    const result = await Swal.fire({
      title: 'Hapus Laporan?',
      text: `Laporan Barang Temuan "${itemName}" akan dihapus permanen.`,
      icon: 'warning',
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      try {
        await foundItemsService.delete(reportId);
    ...
    <div className="flex flex-col gap-6 pb-10">
      ...
      <div className="flex items-center gap-3 border-b border-border pb-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold">Manajemen Laporan Barang Temuan</h1>
          <p className="text-muted-foreground text-sm">
            {reports.length} total laporan barang temuan
          </p>
        </div>
      </div>
              ...
              >
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1">
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        <span className="truncate">{report.lokasi_ditemukan}</span>
                      </div>
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        <Calendar size={13} className="text-primary flex-shrink-0" />
                        <span>
                          {new Date(report.tanggal_ditemukan).toLocaleDateString('id-ID', {
                            day: 'numeric',
                            month: 'short',
                            year: 'numeric',
                          })}
                ...
                {report.status === 'published' && (
                  <div className="flex gap-3 mt-4 pt-4 border-t border-border">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 transition-colors"
                    >
                      <CheckCircle size={16} /> Tandai Sudah Diklaim
                    </button>
                    <button
                ...
                {report.status === 'claimed' && (
                  <div className="flex gap-3 mt-4 pt-4 border-t border-border">
                    ...
                    >
                      <CheckCircle size={16} /> Publikasikan Ulang
                    </button>
                    <button
                      onClick={() => handleDelete(report.id, report.nama_barang)}
                      className="flex items-center justify-center gap-2 px-5 py-2.5 bg-destructive text-destructive-foreground rounded-lg text-sm font-medium hover:bg-destructive/90 transition-colors"
                    >
